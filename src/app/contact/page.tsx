...
function ContactPage() {
  return (
    <div className="bg-gradient-to-b from-dark to-darkest min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center mb-16 animate-fade-in-down">
          <h1 className="text-6xl font-extrabold text-brand mb-4">
            Get in Touch
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div className="bg-lightest rounded-2xl shadow-xl overflow-hidden transform hover:scale-105 transition-transform duration-300">
            <div className="relative h-64">
              <Image
                ...
                objectFit="cover"
              />
              <div className="absolute inset-0 bg-dark bg-opacity-75 flex items-center justify-center">
                <h2 className="text-3xl font-bold text-lightest">Contact Information</h2>
              </div>
            </div>
          ...
          <div className="bg-lightest rounded-2xl shadow-xl p-8 transform hover:scale-105 transition-transform duration-300">
        ...
        <div className="mt-16 bg-light border-l-4 border-brand p-8 rounded-2xl shadow-lg animate-pulse" role="alert">
          <div className="flex items-center">
            <div className="flex-shrink-0">
              <FaBolt className="h-8 w-8 text-brand animate-bounce" />
            </div>
            <div className="ml-4">
        ...
        <div className="mt-16">
          <h2 className="text-3xl font-bold text-brand mb-6">Frequently Asked Questions</h2>
          <FAQ />
        </div>
      </div>
    </div>
...
function ContactItem({ icon, title, content }) {
  return (
    <div className="flex items-start">
      <div className="flex-shrink-0 h-6 w-6 text-brand mt-1">
        {icon}
      </div>
      <div className="ml-3">
        <p className="text-lg font-medium text-dark">{title}</p>
        <p className="mt-1 text-mid">{content}</p>
      </div>
    </div>
  );
}
...
function FAQ() {
  ...
  return (
    <div className="space-y-4">
      {faqs.map((faq, index) => (
        <details key={index} className="bg-lightest rounded-lg shadow-md p-4">
          <summary className="font-semibold text-lg cursor-pointer text-dark">{faq.question}</summary>
          <p className="mt-2 text-mid">{faq.answer}</p>
        </details>
      ))}
    </div>
  );
